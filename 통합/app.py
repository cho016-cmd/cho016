...
def main():
    ...
    if html_code:
        ...
        try:
            ...
            st.error(f"HTML 렌더링 중 오류가 발생했습니다: {str(e)}")
            st.error(f"An error occurred while rendering HTML: {str(e)}")
            st.text("HTML 내용 미리보기:")
            st.text(html_code[:500] + "..." if len(html_code) > 500 else html_code)
